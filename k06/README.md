...
必須問題（１）
l.73 int型の変数i, j, kを宣言する.
l.74 int型の変数new_valueを宣言する.
l.76 ~ l.82 配列nap_valueを初期化する.
l.84 ~ l.105 iの値がitemsの値以下の間for文を繰り返す.
    l.86 ~ l.90 商品が入らなかった場合は上の状態を引き継ぐような処理を行う.
    l.91 ~ l.104 商品を入れ替えた場合の価値を計算する. そのままの場合と入れ替えた場合の価値を比較し, 大きいほうの値を採用するようにする. 
l.107 最大取得可能なカロリーを表示するprintf文を記述.
